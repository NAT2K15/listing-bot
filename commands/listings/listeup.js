...
module.exports.run = async(client, message, args) => {
    ...
    if (config.listings.enable == true) {
        if (!message.member.roles.cache.has(config.listings.allowes_role)) {
            ...
            message.channel.send(`What is the EUP name/pack you are wishing to list?`)
            message.channel.awaitMessages(m => m.author.id == message.author.id, { max: 1 }).then(firstmsg => {
                let eupname = firstmsg.first().content.toLowerCase()
                ...
                message.channel.awaitMessages(m => m.author.id == message.author.id, { max: 1 }).then(secondmsg => {
                    ...
                    message.channel.send(`What is the pack/item going to be coming with/included`)
                                    ...
                                    message.channel.send(`Are you sure you would like to list this product?`).then(msg => {
                                        ...
                                        msg.awaitReactions(filterE, { max: 1, time: ms("20m"), errors: ['time'] }).then(collected => {
                                            ...
                                            if (reaction.emoji.name === '❌') {
                                                ...
                                                message.channel.send(`Eup listing canceled`)
                                            }
                                            if (reaction.emoji.name === '✅') {
                                                let e1 = new MessageEmbed();
                                                e1.setTitle(`${eupname}`)
                                                ...
                                                e1.setThumbnail(message.guild.iconURL({ format: `png`, dynamic: true, size: 1024 }))
...
module.exports.help = {
    name: "listeup",
    description: "This command will list an eup item for you",
    usage: "",
    category: "Info",
    aliases: []
};